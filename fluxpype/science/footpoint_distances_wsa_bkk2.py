...
# --- Polar cap treatment (degrees)
# Pixels within this angular distance from either pole act as seeds for polar CH flood-fill.
_POLAR_SEED_DEG = 10.0
# Optional small bridging near the poles to close fjords before propagation.
_POLAR_BRIDGE_DEG = 4.0

# --- Polar outline trim (degrees): outline pixels with |lat| >= this are dropped
_POLAR_TRIM_DEG = 75.0
...
# --- Helper: contiguous polar caps from open topology ---
def _polar_caps_from_open(open_mask: np.ndarray,
                          lat_1d: np.ndarray,
                          seed_deg: float = _POLAR_SEED_DEG,
                          bridge_deg: float = _POLAR_BRIDGE_DEG) -> np.ndarray:
    """
    Return a boolean mask of CONTIGUOUS polar coronal holes (open regions touching either pole),
    combining a distance-from-pole seed with flood fill and optional local bridging.

    Steps
    -----
    1) Identify seed rows within `seed_deg` of the N/S poles **inside** the open mask.
    2) Lightly 'bridge' the open mask with a single 8-connected closing to seal 1-px fjords near the pole.
    3) Flood-fill (binary_propagation) from seeds restricted to the bridged open mask.
    4) Fill interior holes inside the polar caps.
    """
    st8 = generate_binary_structure(2, 2)
    lat = lat_1d  # radians

    # Distance to north/south poles (colatitudes)
    colat_n = (np.pi / 2.0) - lat
    colat_s = (np.pi / 2.0) + lat

    seed_thr = np.deg2rad(seed_deg)
    seeds_n_rows = colat_n <= seed_thr
    seeds_s_rows = colat_s <= seed_thr

    # Seed masks (only where open)
    seeds_n = np.zeros_like(open_mask, dtype=bool)
    seeds_s = np.zeros_like(open_mask, dtype=bool)
    if seeds_n_rows.any():
        seeds_n[seeds_n_rows, :] = open_mask[seeds_n_rows, :]
    if seeds_s_rows.any():
        seeds_s[seeds_s_rows, :] = open_mask[seeds_s_rows, :]

    # Optional bridging: close tiny fjords globally (simple & robust)
    if bridge_deg and bridge_deg > 0:
        open_bridged = binary_closing(open_mask, st8)
    else:
        open_bridged = open_mask

    # Flood-fill from pole seeds within the (bridged) open mask
    polar_n = binary_propagation(seeds_n, structure=st8, mask=open_bridged)
    polar_s = binary_propagation(seeds_s, structure=st8, mask=open_bridged)
    polar_caps = polar_n | polar_s

    # Fill interior holes so tiny closed islands inside caps don't create boundaries
    polar_caps_filled = binary_fill_holes(polar_caps)

    return polar_caps_filled
...
def _debug_morph_steps(
    ...
    # Outlines for visualization (untrimmed vs trimmed) built from the morphed mask
    outline_untrim = _singleline_outline(B5)
    polar_band_dbg = np.abs(lat_1d) >= np.deg2rad(_POLAR_TRIM_DEG)
    outline_trim = outline_untrim.copy()
    if polar_band_dbg.any():
        outline_trim[polar_band_dbg, :] = False
    ...
    panels = [
        ...
        (f"Region (morphed) + outlines (trim={_POLAR_TRIM_DEG:.1f}°)", region_morph),
    ]
    extent = (lon_1d[0], lon_1d[-1], np.sin(lat_1d[0]), np.sin(lat_1d[-1]))
    for idx, (ax, (title, M)) in enumerate(zip(axes.ravel(), panels)):
        ...
        # draw polar trim latitude lines on every panel
        lat_thr = np.deg2rad(_POLAR_TRIM_DEG)
        y_thr = np.sin(lat_thr)
        ax.axhline(+y_thr, color='k', ls=':', lw=0.8, alpha=0.6)
        ax.axhline(-y_thr, color='k', ls=':', lw=0.8, alpha=0.6)
        # on the last panel, overlay untrimmed vs trimmed outlines
        if idx == len(panels) - 1:
            j_u, i_u = np.where(outline_untrim)
            j_t, i_t = np.where(outline_trim)
            ax.plot(lon_1d[i_u], np.sin(lat_1d[j_u]), '.', ms=1.0, color='orange', alpha=0.7, label='outline (untrimmed)')
            ax.plot(lon_1d[i_t], np.sin(lat_1d[j_t]), '.', ms=1.0, color='cyan',   alpha=0.85, label='outline (trimmed)')
            ax.legend(fontsize='x-small', loc='lower right', framealpha=0.5)
    ...
    if do_plot or force or (ax is not None):
        ...
        for i, cr in enumerate(crs):
            ...
            if os.path.exists(output_file):
                ...
                # --- CONTIGUOUS POLAR CAPS: seed-by-angle + flood-fill inside open regions ---
                open_mask = (P == 0)
                polar_caps = _polar_caps_from_open(open_mask, lat_1d)
                # Union the polar caps into the morphed region so plotted red contour reflects the polar treatment
                region_mask_morph = region_mask_morph | polar_caps

                # Build a clean 1-pixel perimeter for distance queries
                outline_mask = _singleline_outline(region_mask_morph)
                # Remove artificial boundaries within a polar latitude band so poles are distance maxima
                polar_band = np.abs(lat_1d) >= np.deg2rad(_POLAR_TRIM_DEG)
                if polar_band.any():
                    outline_mask[polar_band, :] = False
                # Always drop hard frame edges (top/bottom rows and left/right cols)
                outline_mask[0, :] = False
                outline_mask[-1, :] = False
                outline_mask[:, 0] = False
                outline_mask[:, -1] = False
